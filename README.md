...
当前版本包含可交互开始界面、选关菜单、`Prologue：拼装抽屉`、`Prologue：回声台阶` 和正式 `Chapter 1：寂静的概率花园`。正式第一章的核心是 `搭建 workflow -> 执行 -> 获得 trace 反馈 -> 修改 workflow -> 完成任务`。
...
`chapter_smoke_test.gd` 会实例化序章、回声台阶和正式第一章：验证开始界面、结尾返回、直接选关加载 `P-3`/`回声台阶`/`1-4`，自动走完序章三道门，验证 `R` 重开会清空状态回到 `P-1`，验证三枚回声花瓣会打开终点门，再自动跑完 `WorkflowEvaluator` 和正式第一章四个 workflow 任务。
...
开始界面现在有三项：`开始新游戏`、`选择关卡`、`退出/返回标题`。`开始新游戏` 会按完整顺序进入 `P-1 -> P-2 -> P-3 -> 回声台阶 -> 1-1 -> 1-4 -> Demo 完成`。

`选择关卡` 会列出当前 demo 的全部节点：`P-1/P-2/P-3`、`回声台阶`、`1-1/1-2/1-3/1-4`。所有关卡默认解锁，方便快速测试。直接进入中途关卡时，游戏会自动补齐应有 workflow 节点，例如 `P-3` 会带着 `See/Push/Remember`，`1-4` 会带着正式第一章前面已经获得的全部节点。
